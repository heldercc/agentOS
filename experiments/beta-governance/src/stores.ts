// COPIED from experiments/beta-coding/src/stores.ts (ADR-0012 rig) — rigs are
// disposable science and stay uncoupled; a copy with provenance beats an import.
// File-based stores: reads, hashing, and the write-once artifact guarantee.
// git is the versioning layer (ADR-0010); this module just enforces immutability
// and provenance-by-reference on top of the filesystem.
 ...
 * this is the "Artifacts are immutable once produced" invariant, enforced in code.
 */
...
/**
 * The version of a data file: its short git hash if committed and clean-enough
 * to identify, otherwise a `sha256:<12>` fallback so uncommitted files are still
 * traceable. Never throws — provenance must always resolve to *something*.
 */
